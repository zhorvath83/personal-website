...
########################################################## Stage 1
FROM alpine:3.16.0 AS resume-stage
WORKDIR /resume 
COPY ./resume.json .
RUN apk add npm git
RUN npm init -y
...
########################################################## Stage 2
FROM alpine:3.16.0 AS hugo-stage

# renovate: datasource=github-releases depName=gohugoio/hugo
ENV HUGO_VERSION=v0.100.0
...
########################################################## Stage 3
FROM nginxinc/nginx-unprivileged:1.22.0-alpine AS final-stage
USER root
