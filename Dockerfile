...
########################################################## Stage 1
FROM alpine:3.16.2 AS resume-stage
WORKDIR /resume 
COPY ./resume.json .
RUN apk add npm git
RUN npm init -y
...
########################################################## Stage 2
FROM alpine:3.16.2 AS hugo-stage

# renovate: datasource=github-releases depName=gohugoio/hugo
ENV HUGO_VERSION=v0.104.2
...
########################################################## Stage 3
FROM nginxinc/nginx-unprivileged:1.23.1-alpine AS final-stage
USER root
