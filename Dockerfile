...
########################################################## Stage 1
FROM alpine:3.15.4 AS resume-stage
WORKDIR /resume 
COPY ./resume.json .
RUN apk add npm git
RUN npm init -y
...
########################################################## Stage 2
FROM alpine:3.15.4 AS hugo-stage

# renovate: datasource=github-releases depName=gohugoio/hugo
ENV HUGO_VERSION=v0.99.0
...
########################################################## Stage 3
FROM nginxinc/nginx-unprivileged:1.21.6-alpine AS final-stage
USER root
