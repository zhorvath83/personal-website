...
########################################################## Stage 1
FROM alpine:3.17.1 AS resume-stage
WORKDIR /resume 
COPY ./resume.json .
RUN apk add npm git
RUN npm init -y
...
########################################################## Stage 2
FROM alpine:3.17.1 AS hugo-stage

# renovate: datasource=github-releases depName=gohugoio/hugo
ENV HUGO_VERSION=v0.109.0
...
########################################################## Stage 3
FROM nginxinc/nginx-unprivileged:1.23.3-alpine AS final-stage
USER root
